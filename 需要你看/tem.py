...
# --------------------------
# 优化模式：单次连接+持续读取+数据变化高亮+异常处理（PT100温度专用）
# --------------------------
def single_connect_continuous_read_pt100():
    # 1. 设备参数（务必与设备手册一致！）
    DEVICE_IP = "192.168.1.101"    # 设备IP
    DEVICE_PORT = 8234           # 设备Modbus端口
    SLAVE_ADDR = 1               # 设备从站地址
    FUNC_CODE = 0x04             # 功能码（0x03=保持寄存器，0x04=输入寄存器）
    START_REG = 0                # PT100温度起始寄存器地址（根据设备调整）
    REG_COUNT = 1                # PT100温度读取寄存器数量（1个）
    ...
    print("="*80)
    print("🚀 启动 [PT100温度读取] 模式（单次连接+持续读取）")
    print("="*80)
    print(f"📡 设备地址: {DEVICE_IP}:{DEVICE_PORT}")
    print(f"🔌 从站地址: {SLAVE_ADDR} | 功能码: 0x{FUNC_CODE:02X}")
    print(f"📝 读取范围: 寄存器{START_REG}（共{REG_COUNT}个，PT100温度专用）")
    ...
    def connect_device() -> bool:
        ...
        try:
            ...
            print(f"{GREEN}✅ 连接成功！开始读取PT100温度...{RESET}")
            return True
    ...
    try:
        while True:
            ...
            try:
                ...
                # 数据转换（PT100温度专用公式：原始值÷10 = 实际温度，如193→19.3℃）
                temp_raw = registers[0]
                temperature = temp_raw / 10  # 核心转换公式（根据设备手册调整）
                read_duration = (time.time() - read_start_time) * 1000  # 毫秒

                # 高亮变化数据
                temp_str = f"{temperature:5.1f}℃"
                temp_raw_str = f"{temp_raw:4d}（0x{temp_raw:04X}）"  # 同时显示十进制和十六进制
                ...
                print(f"[{current_time}] ✅ 第{read_count:03d}次 | 耗时:{read_duration:4.0f}ms | "
                      f"PT100原始值:{temp_raw_str} → 温度:{temp_str}")
...
if __name__ == "__main__":
    single_connect_continuous_read_pt100()